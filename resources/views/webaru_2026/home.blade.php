...
@section('content')
    ...
    @php
        $heroSlides = [
            [
                'image' => 'aru-carousel-01.png',
                'alt' => 'มหาวิทยาลัยแห่งเมืองมรดกโลก สร้างคนดี มีปัญญา พัฒนาท้องถิ่น',
            ],
        ];
    ...
    <section id="home" class="relative bg-[#F7F8FA] pt-4 sm:pt-5 lg:pt-6"
        x-data="heroCarousel({ total: {{ count($heroSlides) }}, interval: 6000 })"
        @mouseenter="stop()"
        @mouseleave="start()">
        ...
        <div class="webaru-container">
            <div class="relative overflow-hidden rounded-[22px] border border-black/5 bg-white shadow-[0_22px_70px_rgba(15,23,42,0.08)] lg:rounded-[30px]">
                ...
                {{-- Desktop/tablet: preserve the 1920x720 artwork exactly --}}
                <div class="hidden md:block">
                    <div class="relative aspect-[8/3] w-full overflow-hidden bg-white">
                        @foreach ($heroSlides as $index => $slide)
                            <div x-cloak x-show="active === {{ $index }}" x-transition.opacity.duration.700ms class="absolute inset-0">
                                <img src="{{ asset('images/webaru_2026/hero/' . $slide['image']) }}"
                                    alt="{{ $slide['alt'] }}"
                                    width="1920"
                                    height="720"
                ...
                {{-- Mobile: use the right side of the same artwork as a visual, then render readable HTML content --}}
                <div class="md:hidden">
                    <div class="relative h-[245px] overflow-hidden bg-[#EAF4FC]">
                        <img src="{{ asset('images/webaru_2026/hero/aru-carousel-01.png') }}"
                            alt="วัดและเจดีย์ในจังหวัดพระนครศรีอยุธยา"
                            width="1920"
                            height="720"
                            fetchpriority="high"
                            class="h-full w-full scale-[1.8] object-cover object-[82%_50%]">
                        <div class="absolute inset-0 bg-gradient-to-t from-black/35 via-transparent to-transparent"></div>
                        <div class="absolute bottom-4 left-4 rounded-full border border-white/35 bg-white/90 px-3 py-1.5 text-[11px] font-semibold text-aru-primary shadow-sm backdrop-blur">
                ...
                @if (count($heroSlides) > 1)
                    <div class="absolute bottom-[112px] left-1/2 z-20 hidden -translate-x-1/2 gap-2 md:flex">
                        @foreach ($heroSlides as $index => $slide)
                            <button type="button"
                                class="h-2.5 rounded-full shadow-sm transition-all duration-300"
                                :class="active === {{ $index }} ? 'w-8 bg-aru-primary' : 'w-2.5 bg-white/90'"
                                @click="goTo({{ $index }})"
    ...
    <section class="bg-[#F7F8FA] py-4 lg:py-5">
        <div class="webaru-container">
            <div class="flex min-h-16 items-center gap-4 rounded-2xl bg-gradient-to-r from-[#8E1420] via-aru-primary to-[#B21B2B] px-4 text-white shadow-[0_12px_30px_rgba(142,20,32,0.18)] sm:px-5 lg:px-6">
                <span class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-white/10 text-lg" aria-hidden="true">📢</span>
                ...
                <a href="#" class="min-w-0 flex-1 truncate text-xs text-white/85 transition hover:text-white sm:text-sm">
                    ประกาศมหาวิทยาลัยราชภัฏพระนครศรีอยุธยา เรื่อง การรับสมัครนักศึกษาภาคปกติ ประจำปีการศึกษา 2569
                </a>
                <a href="#" class="shrink-0 rounded-lg px-2 py-1 text-xs font-semibold transition hover:bg-white/10 sm:text-sm">
    ...
    {{-- =========================================================
        NEWS
    ========================================================== --}}
    <section id="news" class="bg-[#F7F8FA] py-12 lg:py-16">
        <div class="webaru-container">
            <div class="mb-7 flex items-end justify-between gap-5 lg:mb-9">
                <div>
                    ...
                    <p class="mt-2 hidden text-sm text-aru-muted sm:block">ติดตามข่าวประชาสัมพันธ์ กิจกรรม และความเคลื่อนไหวของมหาวิทยาลัย</p>
                </div>
                <a href="#" class="inline-flex shrink-0 items-center gap-2 rounded-xl border border-aru-primary/25 bg-white px-4 py-2.5 text-sm font-semibold text-aru-primary shadow-sm transition hover:border-aru-primary hover:bg-aru-primary hover:text-white">
                    ดูทั้งหมด <span aria-hidden="true">→</span>
                </a>
            </div>
            ...
            <div class="grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
                @foreach ($news as $item)
                    <article class="group overflow-hidden rounded-[22px] border border-black/5 bg-white shadow-[0_10px_35px_rgba(15,23,42,0.05)] transition duration-300 hover:-translate-y-1 hover:shadow-[0_18px_45px_rgba(15,23,42,0.10)]">
                        <a href="#" class="relative block aspect-[16/10] overflow-hidden bg-slate-100">
                            <img src="{{ asset('images/webaru_2026/news/' . $item['image']) }}"
                                alt="{{ $item['title'] }}"
                                loading="lazy"
                                class="h-full w-full object-cover transition duration-700 group-hover:scale-105">

                            <div class="absolute left-3 top-3 flex min-w-[52px] flex-col items-center rounded-xl border border-white/70 bg-white/95 px-2.5 py-2 text-center shadow-lg backdrop-blur">
                                <span class="text-lg font-extrabold leading-none text-aru-primary">{{ $item['day'] }}</span>
                                <span class="mt-1 text-[10px] font-semibold text-aru-muted">{{ $item['month'] }}</span>
                            </div>
                        </a>

                        <div class="p-5">
                            <span class="inline-flex rounded-full bg-aru-gold/15 px-2.5 py-1 text-[11px] font-bold text-[#9A6B00]">{{ $item['category'] }}</span>
    ...
    {{-- =========================================================
        FACULTIES
    ========================================================== --}}
    <section id="faculties" class="bg-white py-12 lg:py-16">
        <div class="webaru-container">
            <div class="mb-8 flex items-end justify-between gap-5">
                ...
                <a href="#" class="text-sm font-semibold text-aru-primary transition hover:translate-x-1">ดูทั้งหมด →</a>
            </div>
            ...
            <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
                @foreach ($faculties as $faculty)
                    <a href="#"
                        ...
                        <span class="absolute -right-2 -top-5 text-[72px] font-black leading-none text-aru-primary/[0.045] transition group-hover:text-aru-primary/[0.08]">{{ $faculty['no'] }}</span>

                        <div class="relative flex h-full flex-col">
                            <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-aru-primary/10 text-sm font-extrabold text-aru-primary">{{ $faculty['no'] }}</span>
                            <h3 class="mt-5 text-[15px] font-bold leading-6 text-aru-text group-hover:text-aru-primary">{{ $faculty['name'] }}</h3>
    ...
    <section id="admission" class="bg-white pb-4 pt-4 lg:pb-6 lg:pt-6">
        <div class="webaru-container">
            <div class="relative overflow-hidden rounded-[28px] bg-gradient-to-r from-[#74111B] via-[#A51624] to-[#C42533] text-white shadow-[0_24px_65px_rgba(122,16,28,0.18)] lg:rounded-[32px]">
                <div class="pointer-events-none absolute -right-20 -top-24 h-80 w-80 rounded-full bg-aru-gold/30 blur-3xl"></div>

                <div class="relative grid lg:grid-cols-[0.85fr_1fr_0.95fr]">
                    <div class="relative min-h-[300px] overflow-hidden lg:min-h-[430px]">
                        <img src="{{ asset('images/webaru_2026/admission/admission-students.jpg') }}"
                            alt="สมัครเรียน ARU"
                            loading="lazy"
                            class="absolute inset-0 h-full w-full object-cover">
                        <div class="absolute inset-0 bg-gradient-to-t from-[#74111B]/65 via-transparent to-transparent lg:bg-gradient-to-r lg:from-transparent lg:to-[#74111B]/80"></div>
                    </div>

                    <div class="relative flex items-center px-6 py-10 sm:px-8 lg:px-10 lg:py-12">
                        <div>
                            <span class="inline-flex rounded-full border border-aru-gold/30 bg-aru-gold/15 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.14em] text-aru-gold">Admission 2569</span>
                            ...
                            <ul class="mt-6 space-y-3 text-sm text-white/85">
                                <li class="flex gap-3"><span class="text-aru-gold">✓</span><span>ระดับปริญญาตรี ภาคปกติ / ภาคพิเศษ</span></li>
                                <li class="flex gap-3"><span class="text-aru-gold">✓</span><span>ระดับบัณฑิตศึกษา</span></li>
                                <li class="flex gap-3"><span class="text-aru-gold">✓</span><span>นักศึกษาแบบ Portfolio</span></li>
                            </ul>
                            ...
                            <div class="mt-7 flex flex-wrap gap-3">
                                ...
                                <a href="#" class="inline-flex items-center rounded-xl border border-white/30 bg-white/5 px-5 py-3 text-sm font-semibold transition hover:bg-white hover:text-aru-primary">
                    ...
                    <div class="relative p-5 lg:flex lg:items-center lg:p-7">
                        <div class="w-full rounded-[22px] border border-white/30 bg-white p-6 text-aru-text shadow-2xl shadow-black/10 lg:p-7">
                            ...
                            <div class="mt-5 divide-y divide-aru-border/70 text-sm">
                                <div class="flex items-center gap-3 py-3"><span class="h-2 w-2 rounded-full bg-aru-gold"></span><span>คุณภาพการศึกษาได้มาตรฐาน</span></div>
                                <div class="flex items-center gap-3 py-3"><span class="h-2 w-2 rounded-full bg-aru-gold"></span><span>อาจารย์ผู้เชี่ยวชาญ</span></div>
                                <div class="flex items-center gap-3 py-3"><span class="h-2 w-2 rounded-full bg-aru-gold"></span><span>สิ่งสนับสนุนการเรียนรู้ครบครัน</span></div>
                                <div class="flex items-center gap-3 py-3"><span class="h-2 w-2 rounded-full bg-aru-gold"></span><span>เชื่อมโยงชุมชนและท้องถิ่น</span></div>
                                <div class="flex items-center gap-3 py-3"><span class="h-2 w-2 rounded-full bg-aru-gold"></span><span>โอกาสการทำงานและสร้างอาชีพ</span></div>
                            </div>
                        </div>
                    </div>
                </div>
    ...
    <section class="bg-white py-8 lg:py-10">
        <div class="webaru-container">
            <div class="grid overflow-hidden rounded-[22px] border border-aru-border/70 bg-[#FBFBFC] sm:grid-cols-2 lg:grid-cols-5">
                @foreach ($statistics as $index => $stat)
                    <div class="relative px-5 py-6 text-center lg:py-7 {{ $index > 0 ? 'border-t border-aru-border/70 sm:border-t-0 lg:border-l' : '' }} {{ $index === 2 || $index === 4 ? 'sm:border-t' : '' }}">
                        <div class="text-2xl font-extrabold tracking-tight text-aru-primary lg:text-[28px]">{{ $stat['number'] }}</div>
                ...
                @foreach ($services as $index => $service)
                    <a href="#"
                        ...
                        <div class="flex items-start justify-between gap-3">
                            <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-aru-primary/10 text-sm font-extrabold text-aru-primary transition group-hover:bg-aru-primary group-hover:text-white">0{{ $index + 1 }}</span>
                            <span class="text-aru-muted transition group-hover:translate-x-1 group-hover:text-aru-primary">↗</span>
                        </div>
                        <div class="mt-5">
                            <h3 class="text-sm font-bold text-aru-text group-hover:text-aru-primary">{{ $service['title'] }}</h3>
